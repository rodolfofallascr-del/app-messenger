...
Deno.serve(async (req) => {
  try {
    ...
    for (const batch of batches) {
      ...
      if (!response.ok) {
        return json(502, { error: "Expo push send failed", status: response.status, body: text.slice(0, 300) }, origin);
      }
    ...
    return json(200, { ok: true, sent: expoTokens.length, invalidTokens: invalidTokens.length, result: allResults }, origin);
  } catch (error) {
    const origin = req.headers.get("origin") ?? "";
